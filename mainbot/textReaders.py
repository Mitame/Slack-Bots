...
class youTubeScanner(TextReader):
    ...
    def on_call(self, event, positions):
        ...
        self.bot.send_reply(event, "Video posted by \b%s\b is: \b%s\b by \b%s\b, \b%s\b" % (event.source.nick, title, channel, description))
...
class imgurScanner(TextReader):
    ...
    def on_call(self, event, positions):
        ...
        try:
            description = x.description[77]+"..." if len(x.description) > 80 else x.description
        except TypeError:
            description = None

        text = "Imgur link posted by \b%s\b: " % self.bot.getName(event.user)
        ...
        text = text.replace("\b","*")
        self.bot.send_reply(event, text)
