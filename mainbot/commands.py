...
class Command:
    ...
    def on_fail(self, event):
        self.bot.send_PrivMsg(event.user,
...
class ping(Command):
    ...
    def on_call(self, event, *args):
        self.bot.send_PrivMsg(event.user, "PONG")
...
class cnJoke(Command):
    ...
    def on_call(self, event, *args):
        ...
        try:
            a = json.loads(z)
            self.bot.send_reply(event, a["value"]["joke"])
        except ValueError:
            print(z)
    
    def on_fail(self,event):
        self.bot.send_PrivMsg(event.user,
                              "You failed to type the command correctly puny human. \nChuck Norris will roundhouse kick you in the face shortly.")
    ...
    def on_call(self, event, *args):
        ...
        if args[0] == "create":
            ...
            try:
                ...
                self.bot.send_PubMsg("No poll to close.")
        else:
            if args[0].isdecimal():
...
class help(Command):
    ...
    def on_call(self, event, *args):
        ...
        for x in list(self.bot.commands.items()):
            print(x)
            if x[1].permissionLevel <= self.bot.getPermLevel(event.user):
                commands.append(x[0])
        
        commands.sort()
        self.bot.send_PrivMsg(event.user, "---Commands avaliable to you---\n" +
